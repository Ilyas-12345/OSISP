...
int custom_compare(const struct dirent **a, const struct dirent **b) {
...
void process_directory(const char* directory, const struct Options* opt) {
    ...
    num_entries = opt->s ? scandir(directory, &entries, NULL, custom_compare) : scandir(directory, &entries, NULL, NULL);
